...
The `Authorization` header references the `GENUDO_API_KEY` environment variable
...
```bash
export GENUDO_API_KEY="gnd_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
```

For Claude Code on the web, add `GENUDO_API_KEY` as an environment secret in your
...
> the `GENUDO_API_KEY` is set and this config is in place, reload / start a new
